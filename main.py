...
@app.post("name/{name}/surname/{surname}", consumes=[Mime.FormData], controller=userController)
def bar(req, res, var):
    query_params_string = ""
    path_params_string = ""
    ...
    #print(req.content["key"])
    #print(req.content.key)
    #print(req.content.multilinekey)
    #print(type(req.content.file))
    result = """Path variable jede
    <br> QueryParams: <br>"""+query_params_string+"""
    <br> Path params: <br>"""+path_params_string+"""
    """
    res.status(Status.ACCEPTED).type(Mime.HTML).entity(result).build()
